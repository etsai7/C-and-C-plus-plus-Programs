...
int init_ListHead();
int Insert_Unsorted( int insert_key, int insert_data);
...
//        printf("%9c\t%8d\t%8d\n", r_op, r_key, r_data );
	...
	if(r_op == 'I')
	{
		Insert_Unsorted(r_key,r_data);		
	}   
	else if(r_op == 'D')
	{
		Delete(r_key);
//		printf("CALLING DELETE RIGHT NOW");
//		Print();
	}
	else if(r_op == 'G')
	{
...
//Inserting unsorted a specific key and data if possible -------------------------------------
int Insert_Unsorted(int insert_key, int insert_data)
{
    record *dummy_record;
    ...
    dummy_record->key = insert_key;
    dummy_record-> data = insert_data;
...
//Case 3: Add it in unsorted to the end.
...
/*	else if(tempList->next == NULL && tempList->key < insert_sorted_key)
	{
		tempList->next = dummy_record;
		next_count++;
		return 1;
